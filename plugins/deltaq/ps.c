...
struct qos_vals_node * getPortQos(struct int_rmt_data * ps, TPortId port_id, 
	...
	for(struct port_vals_node * it_port = ps->port_vals_begin; 
		...
		if(it_port->key == port_id) {
			...
			for(struct qos_vals_node * it_qos it_qos = 
...
struct port_vals_node * getPort(struct int_rmt_data * ps, TPortId port_id) {
	for(port_vals_node * it_port = ps->port_vals_begin; it_port!= NULL; 
...
struct size_time_node * getSizeTimeNode(struct int_rmt_data * ps, TSyze size, 
		TTime time, struct size_time_node * next) {
	struct size_time_node * ret = NULL;
	if(ps->buffer_size_time_begin == NULL) {
		ret = (struct size_time_node *) 
			kmalloc(sizeof(struct size_time_node), GFP_ATOMIC);
	} else {
...
struct time_node * getTimeNode(struct int_rmt_data * ps, 
		TTime time, struct time_node * next) {
...
struct pdu_node * getPDUNode(struct int_rmt_data * ps, pdu * p, 
		struct pdu_node * next) {
...
int imp_rmt_limiter_rx(struct rmt_ps * _ps, struct rmt_n1_port * _port, 
	...
	for(struct size_prob_node * it_dprob = 
		...
		it_dprob != NULL && it_dprob->size <= next_size; 
	...
	qosVals->received_size_time_begin = getSizeTimeNode(ps, pdu_size, 
		time, NULL);
	qosVals->received_size_time_end = qosVals->received_size_time_begin;
	} else {
	qosVals->received_size_time_end->next = getSizeTimeNode(ps, pdu_size, 
		time, NULL);
...
int imp_rmt_enqueue_tx (struct rmt_ps * _ps, struct rmt_n1_port * _port, 
	...
	for(struct size_prob_node * it_dprob = 
		...
		it_dprob != NULL && portVals->count <= qosVals->received_size; 
		it_dprob = it_dprob->next) {
			dropProb = it_dprob->prob;
	}
...
struct pdu * imp_rmt_dequeue_tx(struct rmt_ps * _ps , 
	...
	for(struct qos_vals_node * it_qos = portVals->qos_vals_begin; 
		...
		if(it_qos->params->limit_out) {
			...
			if(it_qos->send_size + 
				it_qos->queue_begin->p->buffer->size > 
				it_qos->params->out_burst_size ) { continue; }
		}

	/*Compute t_hop_delay as (max expected + insert time - current time) 
		[*multiplier <if negative>]*/
		TTime t_hop_delay = it_qos->params->out_expected_waiting_time 
			- time + it_qos->wtime_begin->time ;
		if(t_hop_delay < 0) {
			t_hop_delay *= 
			it_qos->params->
				out_multiplier_over_expected_waiting_time;
		} else {
			t_hop_delay *= 100;
		}
	...
	if(ret_qos->send_size_time_begin == NULL) {
		ret_qos->send_size_time_begin = 
			getSizeTimeNode(ps, ret->buffer->size,NULL);
 ...
 */
int imp_rmt_requeue_tx(struct rmt_ps * _ps, struct rmt_n1_port *, 
		struct pdu *) {
	printk("%s: called()\n", __func__);
	struct int_rmt_data * ps = container_of(_ps, struct int_rmt_data, base);
...
rmt_ps_deltaq_create(struct rina_component * component) {
	...
	ps->base.rmt_scheduling_create_policy_tx = 
		imp_rmt_scheduling_create_policy_tx;
	ps->base.rmt_scheduling_destroy_policy_tx = 
		imp_rmt_scheduling_destroy_policy_tx;
