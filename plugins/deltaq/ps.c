...
struct qos_params_node {
	...
	/*QoS Limits Input?*/
	bool limit_in;
	/*Burst duration*/TTime in_burst_time;
	/*Probability of drop if burst size surpassed*/
	struct size_prob_node * in_burst_size_drop_prob_begin;

	/*QoS Limits Output?*/
	bool limit_out;
	/*Burst duration*/TTime out_burst_time;
	/*Wait if burst size surpassed on burst*/TSyze out_burst_size;

	/*Probability of cherish drop*/
	struct size_prob_node * out_drop_prob_begin;
	/*Expected max time waiting on queue*/TTime out_expected_waiting_time;
	/*Multiplier when real waiting time surpases max expected*/
	int out_multiplier_over_expected_waiting_time;
	/*[100 if not applies]*/
...
struct qos_vals_node * getPortQos(struct int_rmt_data * ps, TPortId port_id,
		TQoSId qos_id, struct port_vals_node** ret_port) {
	for (struct port_vals_node * it_port =
			ps->port_vals_begin; it_port != NULL;
			it_port = it_port->next) {
		if (it_port->key == port_id) {
			if (ret_port != NULL) {
				*ret_port = it_port;
			}
			for(struct qos_vals_node * it_qos =
					it_port->qos_vals_begin; it_qos != NULL;
					it_qos = it_qos->next
			) {
...
struct port_vals_node * getPort(struct int_rmt_data * ps,
		TPortId port_id) {
	for (struct port_vals_node * it_port = ps->port_vals_begin;
			it_port != NULL; it_port = it_port->next) {
		if (it_port->key == port_id) {
			return it_port;
		}
	}
...
struct size_time_node * getSizeTimeNode(struct int_rmt_data * ps,
		TSyze size, TTime time, struct size_time_node * next) {
	struct size_time_node * ret = NULL;
	if (ps->buffer_size_time_begin == NULL) {
		ret = (struct size_time_node *) kmalloc(
				sizeof(struct size_time_node), GFP_ATOMIC);
	} else {
...
void releaseSizeTimeNode(struct int_rmt_data * ps,
		struct size_time_node * node) {
	node->next = ps->buffer_size_time_begin;
	ps->buffer_size_time_begin = node;
}

struct time_node * getTimeNode(struct int_rmt_data * ps, TTime time,
		struct time_node * next) {
	struct time_node * ret = NULL;
	if (ps->buffer_time_begin == NULL) {
		ret = (struct time_node *) kmalloc(sizeof(struct time_node),
				GFP_ATOMIC);
	} else {
...
struct pdu_node * getPDUNode(struct int_rmt_data * ps, struct pdu * p,
		struct pdu_node * next) {
	struct pdu_node * ret = NULL;
	if (ps->buffer_pdu_begin == NULL) {
		ret = (struct pdu_node *) kmalloc(sizeof(struct pdu_node),
				GFP_ATOMIC);
	} else {
 ...
 * IF limit_drop_prob >= 1 DROP, RETURN -1;
		 if limit_drop_prob <= 0 ACCEPT;
		 (random < limit_drop_prob)? ACCEPT : DROP, RETURN -1;
 * INSERT {time, PDU.size} INTO QoS_PORT-received_size_time
 * ADDS PDU.size TO QoS_PORT-received_size
 */
int imp_rmt_limiter_rx(struct rmt_ps * _ps, struct rmt_n1_port * _port,
		struct pdu * p) {
	printk("%s: called()\n", __func__);
	...
	struct qos_vals_node * qosVals = getPortQos(ps, port_id, qos_id,
			NULL);
	...
	if (it_stime == NULL) {
		qosVals->received_size_time_end = NULL;
	}

	TSyze pdu_size = p->buffer->size;

	/*Get drop probability. [0-10.000]*/TSyze next_size =
			qosVals->received_size + pdu_size;
	int dropProb = 0;
	for (struct size_prob_node * it_dprob =
			qosVals->params->in_burst_size_drop_prob_begin;
	...
	if (qosVals->received_size_time_begin == NULL) {
		qosVals->received_size_time_begin = getSizeTimeNode(ps,
				pdu_size, time, NULL);
		qosVals->received_size_time_end =
				qosVals->received_size_time_begin;
	} else {
		qosVals->received_size_time_end->next = getSizeTimeNode(ps,
				pdu_size, time, NULL);
		qosVals->received_size_time_end =
				qosVals->received_size_time_end->next;
	}
 ...
 * IF count_drop_prob >= 1 DROP, RETURN -1; if count_drop_prob <= 0 ACCEPT;
		 (random < count_drop_prob)? ACCEPT : DROP, RETURN -1;
 * INCREMENT PORT-count
 * PORT-QOS-queue PUSH BACK pdu
 * GET time
 * PORT-QOS-wtime PUSH BACK stime
 */
int imp_rmt_enqueue_tx(struct rmt_ps * _ps, struct rmt_n1_port * _port,
		struct pdu * p) {
	printk("%s: called()\n", __func__);
	struct int_rmt_data * ps =
	...
	int dropProb = 0;
	for (struct size_prob_node * it_dprob =
			qosVals->params->out_drop_prob_begin;
			it_dprob != NULL
					&& portVals->count <= qosVals->received_size;
	...
	/*STORE time log*/TTime time = ktime_get();
 ...
 * RETURN pdu
 */
struct pdu * imp_rmt_dequeue_tx(struct rmt_ps * _ps,
	...
	if (portVals == NULL) {
		LOG_WARN(
				"Port not initialicied for given input port structure.");
		return -1;
	}
	...
	for (struct qos_vals_node * it_qos = portVals->qos_vals_begin;
			it_qos != NULL; it_qos = it_qos->next) {
		...
		if (it_qos->params->limit_out) {
			...
			struct size_time_node * it_stime =
					it_qos->send_size_time_begin;
			...
			if (it_stime == NULL) {
				it_qos->send_size_time_end = NULL;
			}

			/*Skip if next PDU exceeds burst size*/
			if (it_qos->send_size + it_qos->queue_begin->p->buffer->size
					> it_qos->params->out_burst_size) {
				continue;
			}
		}

		/*Compute t_hop_delay as (max expected + insert time - current time)
				 [*multiplier <if negative>]*/TTime t_hop_delay =
						 it_qos->params->out_expected_waiting_time - time
						 + it_qos->wtime_begin->time;
				 if (t_hop_delay < 0) {
					 t_hop_delay *=
							 it_qos->params->out_multiplier_over_expected_waiting_time;
				 } else {
					 t_hop_delay *= 100;
				 }
	...
	/*Store next serving pdu insert time as last served and
			 remove from log*/
	...
	if (ret_qos->params->limit_out) {
		if (ret_qos->send_size_time_begin == NULL) {
			ret_qos->send_size_time_begin = getSizeTimeNode(ps,
					ret->buffer->size, NULL);
			ret_qos->send_size_time_end = ret_qos->send_size_time_begin;
		} else {
			ret_qos->send_size_time_end->next = getSizeTimeNode(ps,
					ret->buffer->size, NULL);
			ret_qos->send_size_time_end =
					ret_qos->send_size_time_end->next;
		}
 ...
 */
int imp_rmt_requeue_tx(struct rmt_ps * _ps, struct rmt_n1_port * port,
		struct pdu * p) {
	printk("%s: called()\n", __func__);
	struct int_rmt_data * ps =
			container_of(_ps, struct int_rmt_data, base);
	...
	/*RE-STORE time log*/
	qosVals->wtime_begin = getTimeNode(ps, qosVals->last_stime,
			qosVals->wtime_begin);
	...
	if (qosVals->params->limit_out) {
		...
		qosVals->send_size_time_begin =
				qosVals->send_size_time_begin->next;
...
rmt_ps_deltaq_create(struct rina_component * component) {
	...
	ps->base.rmt_scheduling_create_policy_tx =
			rmt_ps_deltaq_create;
	ps->base.rmt_scheduling_destroy_policy_tx =
			rmt_ps_deltaq_destroy;
	...
	while (ps->port_vals_begin != NULL) {
		...
		while (t_port->qos_vals_begin != NULL) {
			...
			while (t_qos->received_size_time_begin != NULL) {
				struct size_time_node * t_node =
						t_qos->received_size_time_begin;
